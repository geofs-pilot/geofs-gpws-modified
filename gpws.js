...
if (flight.recorder.playing) {
    ...
    setInterval(() => {
        let geofsSoundOn = geofs.preferences.sound;
        if (!geofsSoundOn) {
            for (let key in window) {
            if (window[key] instanceof Audio) {
                window[key].volume = 0;
            }
        }
        } else {
